...
export default function Home() {
  ...
  const handleHandUpdate = useCallback((data: HandData) => {
    ...
    if (data.isDetected) {
        ...
        // Update debug UI occasionally (not every frame to avoid react render lag)
    ...
    <main className="relative w-screen h-screen overflow-hidden bg-black selection:bg-none">
      ...
      <div className="absolute top-0 w-full p-6 flex justify-between items-start pointer-events-none z-20">
        ...
        <div className="text-right space-y-1">
            <div className="flex items-center gap-2 justify-end">
                <span className={`w-2 h-2 rounded-full ${debugGesture !== 'none' ? 'bg-green-500 shadow-[0_0_10px_#22c55e]' : 'bg-red-500'}`}></span>
                <span className="text-white/60 text-xs font-mono uppercase">AI Vision</span>
            </div>
            <div className="text-white/40 text-xs font-mono">
                MODE: <span className="text-cyan-400">{debugGesture.toUpperCase()}</span>
            </div>
        </div>
      </div>
      ...
      <div className="absolute top-1/2 left-8 transform -translate-y-1/2 space-y-6 pointer-events-none opacity-50 hidden md:block">
          <div className="flex items-center gap-4">
              <div className="w-8 h-8 border border-white/20 rounded flex items-center justify-center text-white/60 text-xs">✋</div>
              <span className="text-white/40 text-xs uppercase tracking-widest">Repulse</span>
          </div>
          <div className="flex items-center gap-4">
              <div className="w-8 h-8 border border-white/20 rounded flex items-center justify-center text-white/60 text-xs">✊</div>
              <span className="text-white/40 text-xs uppercase tracking-widest">Gravity</span>
          </div>
          <div className="flex items-center gap-4">
              <div className="w-8 h-8 border border-white/20 rounded flex items-center justify-center text-white/60 text-xs">👌</div>
              <span className="text-white/40 text-xs uppercase tracking-widest">Condense</span>
          </div>
      </div>
    </main>
  );
